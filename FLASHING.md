...
2. **`&bootloader` 鍵永遠只能讓「按下它的那一側」進 bootloader。**
   2026-09-01 起這變成好事：左半是 central、也是最常刷的板子。

   BT 層（12）左右各有一顆 `&bootloader`，**按哪一邊就重置哪一邊**：
   位置 29 = 左手 `B` ⇒ 左半；位置 30 = 右手 `N` ⇒ 右半。

   進去的和絃（2026-09-24 核對現行 keymap）：

   > 按住**右拇指最內側 `BSPC`**（`&lt 2 BSPC` ⇒ NUM）
   > → 按住**右手底排 `/`**（位置 34 的 `&mo 12` ⇒ BT）
   > → 按左手 `B`

   ⚠️ 這段在 2026-09-24 之前寫的是「按住左拇指 `SPACE` 進 MEDIA、再按住右拇指
   `RET` 進 BT」，**那是舊 keymap**，照著按進不去。BT 的入口在 2026-09-04 那次
   17 → 13 層重整時就搬到 NUM 了（見 `config/toucan.keymap` layer_2 的註解），
   這份文件漏改。要再確認就直接查 keymap：`&mo 12` 在哪一層哪一格。

   右半除了上面那顆 `N`，也可以用它自己 XIAO 上的 RST 鈕（USB-C 接頭旁、絲印
   `RST`，約 2mm，要用迴紋針或鑷子尖）連按兩下。
