#!/usr/bin/env bash
# Build the Toucan *dongle* firmware locally, in Docker.
#
...
#
# Only the dongle is built: in dongle mode it is the split central, so it is the
# only image that carries the keymap. The halves keep the firmware they have.
set -euo pipefail
...
SNIPPET="studio-rpc-usb-uart"   # without this ZMK Studio cannot reach the dongle
OUT="toucan_dongle-${BOARD}-zmk.uf2"
...
west build -s zmk/app -d /ws/build/dongle -b '${BOARD}' -S '${SNIPPET}' -- \
  -DZMK_CONFIG=/ws/config \
  -DSHIELD='${SHIELD}' \
  -DZMK_EXTRA_MODULES=/repo \
  -DCONFIG_ZMK_STUDIO=y

cp /ws/build/dongle/zephyr/zmk.uf2 '/ws/${OUT}'
ls -l '/ws/${OUT}'
"
